...
router.post('/register', bodyParser.json(), async (req, res) => {
  ...
  if (organization.length < 3) {
    return res
      ...
      .json({
        errors: {
          organization: 'Organization must be at least 3 characters long',
        },
      });
  }
  ...
  if (!/^[\w-]+(.[\w-]+)*@([\w-]+.)+[a-zA-Z]{2,7}$/.test(email)) {
    return res
      .status(400)
      .json({ errors: { email: 'Email is not a valid email address' } });
  }
  ...
  try {
    ...
    const insertQuery =
      'INSERT INTO login (UserName, Password, Email,Organization, PhoneNo, Salt) VALUES (?, ?, ?, ?, ?, ?)';
