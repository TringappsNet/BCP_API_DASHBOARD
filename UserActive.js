...
router.put('/', async (req, res) => {
  ...
  try {
    ...
    if (!email || isActive === undefined || isActive === null) {
      return res
        .status(400)
        .json({ error: 'Email and isActive parameter are required!' });
    }
    ...
    return res
      .status(200)
      .json({ message: 'User isActive status updated successfully' });
  } catch (error) {
    console.error('Error updating user isActive status:', error);
    return res
      .status(500)
      .json({ error: 'Error updating user isActive status' });
  }
});
